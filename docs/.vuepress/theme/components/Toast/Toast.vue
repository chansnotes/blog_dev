<template>
    <div
      id="toast"
      class="fixed inset-x-0 m-auto bg-green-500 text-center rounded whitespace-no-wrap overflow-hidden"
      :class="showToast ? 'show' : 'hidden'"
      style="max-width: 50px; height: 50px; top: 50px"
    >
      <div id="img" class="flex justify-center items-center float-left p-2 bg-green-500 rounded box-border" style="width: 50px; height: 50px;">
...
<div class="p-4 overflow-hidden whitespace-no-wrap text-white">{{ toastMessage }}
          </div>
          </div>
    </div>
</template>
...
#toast.show {
    visibility: visible;
    -webkit-animation: fadein 0.5s, expand 0.5s 0.5s,stay 1.5s 1s, shrink 0.5s 2.5s, fadeout 0.5s 3s;
    animation: fadein 0.5s, expand 0.5s 0.5s,stay 1.5s 1s, shrink 0.5s 2.5s, fadeout 0.5s 3s;
}
